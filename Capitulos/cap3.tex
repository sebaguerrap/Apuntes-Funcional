...
\begin{fcorollary}
    \[\sigma_N f\xrightarrow[N\to\infty]{\text{unif}} f\text{ para }f\in C(\Pi)\]
...
\begin{proof}[Demsotración del teorema \ref{theo:3.5.6}]
    ...
    \begin{align*}
        \implies |K_n*f(x)-f(x)|&\leq \int_\Pi |f(x-y)-f(x)||K_n(y)|\,dy\\
        &=\int_{|y|<\delta} |f(x-y)-f(x)||K_n(y)|\,dy+\int_{|y|>\delta} |f(x-y)-f(x)||K_n(y)|\,dy\\
        &\leq \varepsilon\int_\mathbb{T} |K_n(y)|\,dy+2\max_\Pi |f|\int_{|y|>\delta}|K_n(y)|\,dy\\
        &\leq C\varepsilon
    \end{align*}
...
\begin{fcorollary}
    Si $f\in C(\Pi)$ y $\hat f(n)=0\ \forall n\in\Z\implies f\equiv 0$.
\end{fcorollary}
...
\begin{fcorollary}
    Suponga que $f\in C(\Pi)$ y su serie de Fourier converge absoluta y uniformemente, es decir:
...
\begin{proof}
    ...
    \[g(x):=\sum_{n\in\Z}\hat f(n)e_n(x)\in C(\Pi)\]
...
Tenemos la convergencia $\sigma_N f\xrightarrow{\text{unif}} f$ para $f\in C(\Pi)$. Queremos pasar a convergencia en $L^2$. Vamos a utilizar la \textbf{densidad} de $C(\Pi)\subseteq L^2(\Pi)$. Vamos a necesitar la estimación adicional:
...
\begin{fproposition}
    Si $f\in L^p(\Pi)$, $1\leq p<\infty$, entonces 

    \[||\sigma_N f||_{L^p}\leq ||f||_{L^p}\]
\end{fproposition}

\begin{ftheorem}
    Sea $f\in L^p(\Pi)$, $1\leq p<\infty$. Entonces,

    \[\sigma_N f\xrightarrow{L^p} f\]
\end{ftheorem}

\begin{proof}
    Fije $\varepsilon>0$. Aproxime $f\in L^p(\Pi)$ con $g\in C(\Pi)$:
    ...
    \begin{align*}
        ||h||_{L^p}&=\left(\int_\Pi |h|^p\,dx\right)^{1/p}\\
        &\leq \left(\int_\Pi \varepsilon^p\,dx\right)^{1/p}=(2\pi)^{1/p}\varepsilon
    \end{align*}
\end{proof}
...
\begin{flemma}[Riemann-Lebesgue]
    $f\in L^1(\Pi)$, $\hat f(n)=\frac{1}{2\pi}\int_\Pi f(x)e^{-inx}\,dx\xrightarrow{n\to\infty} 0$
\end{flemma}
...
\begin{align*}
    L^2(\Pi)&\to \ell_\Z^2=\{(\ldots,a_{-1},a_0,a_1,\ldots):\sum_{k\in\Z}|a_k|^2<\infty\}\\
...
\begin{align*}
    L^1(\Pi)&\xrightarrow{\mathcal{F}} \hat c_0=\{(\ldots,a_{-1},a_0,a_1,\ldots):\lim_{|n|\to\infty} a_n=0\}\\
    f&\to \hat f
\end{align*}

\begin{ftheorem}
    $L^1(\Pi)\xrightarrow{\mathcal{F}} \hat c_0$ es lineal, acotado e inyectivo.
\end{ftheorem}
...
\begin{proof}
    ...
    pero $\mathcal{F}$ no es sobreyectiva.
\end{proof}